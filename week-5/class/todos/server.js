...
//   - 인증: JWT Bearer 토큰. 모든 /api/todos 라우트는 토큰의 user 로 스코프된다(남의 todo 접근 불가).
// ============================================================================
...
//      클라이언트가 new Date(createdAt) 또는 숫자 정렬을 하므로, ISO 문자열/bigint-문자열을
...
async function me(req, res, authUser) {
  const result = await pool.query('SELECT id, email FROM users WHERE id = $1', [authUser.id]);
  if (result.rows.length === 0) {
...
// ---------------------------------------------------------------------------
// 6.5) API 핸들러 — 할 일 (전부 Bearer 보호, authUser.id 로 스코프)
// ---------------------------------------------------------------------------

// GET /api/todos → 200 [{ id, title, done, createdAt }]  (최신순: created_at DESC, id DESC)
async function listTodos(_req, res, authUser) {
  ...
  sendJSON(res, 200, rows.map(mapTodo));
}

// POST /api/todos  { title } → 201 { id, title, done, createdAt }  (title 공백이면 400)
async function createTodo(req, res, authUser) {
  const body = await readJSONBody(req);
  const title = typeof body.title === 'string' ? body.title.trim() : '';
  if (!title) {
    return sendJSON(res, 400, { error: '할 일 내용(title)은 필수입니다.' });
  }
  ...
  sendJSON(res, 201, mapTodo(rows[0]));
}

// PATCH /api/todos/:id  { done?:boolean, title?:string } → 200 { id, title, done, createdAt }
//   본인 소유가 아니거나 없으면 404. 부분 업데이트(둘 다 생략 시 변경 없이 현재 행 반환).
async function updateTodo(req, res, authUser, id) {
  const body = await readJSONBody(req);
  ...
  if (Object.prototype.hasOwnProperty.call(body, 'title')) {
    const title = typeof body.title === 'string' ? body.title.trim() : '';
    if (!title) {
      return sendJSON(res, 400, { error: '할 일 내용(title)은 비어 있을 수 없습니다.' });
    }
    sets.push(`title = $${n++}`);
    params.push(title);
  }
  if (Object.prototype.hasOwnProperty.call(body, 'done')) {
    if (typeof body.done !== 'boolean') {
      return sendJSON(res, 400, { error: 'done 은 boolean 이어야 합니다.' });
    }
    sets.push(`done = $${n++}`);
    params.push(body.done);
  }

  // 변경할 필드가 없으면 소유권만 확인하고 현재 행을 반환(없으면 404).
  if (sets.length === 0) {
    const cur = await pool.query(
      'SELECT id, title, done, created_at FROM todos WHERE id = $1 AND user_id = $2',
      [id, authUser.id]
    );
    if (cur.rows.length === 0) {
      return sendJSON(res, 404, { error: '해당 할 일을 찾을 수 없습니다.' });
    }
    return sendJSON(res, 200, mapTodo(cur.rows[0]));
  }

  // WHERE 에 user_id 를 함께 걸어 남의 todo 는 매칭 자체가 안 되게 한다(404).
  ...
  sendJSON(res, 200, mapTodo(rows[0]));
}

// DELETE /api/todos/:id → 200 { ok:true }  (본인 소유 아니거나 없으면 404)
async function deleteTodo(_req, res, authUser, id) {
  const { rows } = await pool.query(
...
async function handleApi(req, res, pathname) {
  ...
  // --- 할 일 컬렉션: /api/todos (Bearer) ---
  if (pathname === '/api/todos') {
    const authUser = requireAuth(req, res);
    if (!authUser) return; // 401 이미 응답됨
    if (method === 'GET') return ensureDB().then(() => listTodos(req, res, authUser));
    if (method === 'POST') return ensureDB().then(() => createTodo(req, res, authUser));
    return sendJSON(res, 405, { error: 'Method Not Allowed' });
  }

  // --- 할 일 단건: /api/todos/:id (Bearer) — PATCH / DELETE ---
  ...
  if (m) {
    ...
    if (id === null) return sendJSON(res, 400, { error: '유효하지 않은 할 일 id 입니다.' });
    if (method === 'PATCH') return ensureDB().then(() => updateTodo(req, res, authUser, id));
    return ensureDB().then(() => deleteTodo(req, res, authUser, id));
  }
...
function start() {
  server.listen(PORT, () => {
    console.log(`[server] 할 일 관리 백엔드 실행 → http://localhost:${PORT}`);

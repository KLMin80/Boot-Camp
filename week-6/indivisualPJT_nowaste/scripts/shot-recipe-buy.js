// RecipeSheet의 양념 코멘트 + '🛒 장보기' 버튼 배치를 눈으로 확인하는 스크린샷.
const { chromium } = require('playwright-core');
const { pool } = require('../db');
const CHROME = 'C:/Program Files/Google/Chrome/Application/chrome.exe';
const BASE = 'http://localhost:3300';
const OUT = require('path').join(require('os').tmpdir(), 'nowaste-recipe-buy.png');
const EMAIL = `shot_${Date.now()}@t.com`;
...
(async () => {
  ...
  for (const ig of ['두부', '대파', '김치', '계란', '돼지고기']) await api('POST', '/api/items', { token, body: { name: ig, ingredient: ig, capacity: 300, unit: 'g', price: 3000, storage: 'fridge' } });

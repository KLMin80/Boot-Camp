...
async function generate({ focus, invNames, vocab, tag, need, avoid, must = [] }) {
  ...
  const sys = [
    ...
    '- 소금·간장·참기름 같은 양념은 uses가 아니라 seasonings에 넣되, **각 항목에 양을 함께 적는다** (예: "간장 1큰술", "고춧가루 1큰술", "다진마늘 0.5큰술", "소금 약간"). 큰술/작은술/약간처럼 초보도 아는 단위로.',
    ...
    '- **steps는 4~6단계. 요리 경험이 전혀 없는 자취생도 그대로 따라 할 수 있게** 쓴다. 각 단계에 재료·양념 양, 불세기(약불/중불/센불), 시간, 완성 신호(예: "가장자리가 노릇해지면", "국물이 끓어오르면")를 넣고, 어려운 조리용어는 풀어서 설명한다.',
...
async function byName({ dish, inventory, vocab, tag }) {
  ...
  const sys = [
    ...
    '- 양념(소금·간장 등)은 seasonings에 **양까지** 적는다 (예: "간장 1큰술", "소금 약간"), 사전에 있는 주재료만 uses에.',
    '- **steps는 4~6단계, 요리 초보(자취생)도 따라 하게** 재료·양념 양, 불세기(약/중/센불), 시간, 완성 신호를 구체적으로.',

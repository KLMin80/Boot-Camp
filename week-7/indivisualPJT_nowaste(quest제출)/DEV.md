...
### ✅ OCR 실측 완료 (2026-07) — Tesseract 폐기, GPT-4o-mini 비전 채택

Playwright로 실제 사용자 라벨 사진 9장(네이버 지식인·블로그·뉴스)을 모아 두 엔진 비교(`scripts/ocr-lab.js`, 결과 `ocr-accuracy-test.png`):

| 엔진 | 날짜 인식 | 판정 |
|------|:---:|------|
| **Tesseract.js** | **0/7** | ❌ 폐기. 실제 잉크젯·곡면·마트라벨서 신뢰도 31~56, 쓰레기 텍스트만. 합성 라벨(깨끗한 인쇄)만 읽음 |
| **GPT-4o-mini 비전** | **6/7** | ✅ 채택. 비닐 싼 고기 마트라벨(제품명+용량+날짜)까지. 못 읽으면 **null**(환각 안 함) |

- **비용:** `detail:high`로 장당 ~4원(장바구니 15개 ≈ 60원). ⚠️ `detail:low`(2,868토큰 고정)는 큰 글씨만 되고 **작은 라벨서 날짜를 지어냄**(실측 확인) → high 고정. "찍어두고 나중에 확인" 흐름이라 지연 무관, 확인 화면이 안전망.
- **`server.js`가 프록시**하므로 키 노출 없음. 온디바이스 OCR(Capacitor Vision/ML Kit)은 Phase 5 비용절감 옵션으로 남김.

### ⚠️ 실측이 드러낸 설계 요구사항

- **한국 날짜 도장은 연도가 없다.** `05.08` `02.05` `04.12` — 실사진 대부분 월.일만. → 앱이 **연도 추론**: 월.일이 오늘보다 과거면 내년, 아니면 올해. **필수.**
- **유통기한/제조일 둘 다 있으면 더 나중 날짜**를 유통기한으로 (프롬프트로 지시, 실측서 동작 확인).
- **빈 라벨 양식이 모델을 검증함:** 마트 라벨의 "보관방법 상온·냉장·냉동"이 우리 `storage` 값과 정확히 일치.
- **사진 안내(정확도에서 도출, UI에 반영):** 날짜가 프레임 채우게 가까이 / 유통기한에 초점 / 빛 반사 피하기 / 병은 날짜 정면 / 연도 없어도 OK(앱이 채움).
...
| POST | `/api/buy-links` | **멀티 마켓 딥링크**(쿠팡·컬리·SSG·네이버) + 네이버 참고가(키 있을 때). 재료별 링크 생성 |
...
- [x] 📌 `git commit` — Phase 1 세이브 포인트 (Phase 2·2.5와 함께 `f9b85ea`)
...
- [x] 📌 `git commit` — `f9b85ea` (2026-07-14 푸시)

### Phase 3: 핵심 & 어려운 기능 (불확실한 것부터)
- [x] 🔴 **OCR 정확도 실측** ✅ — Tesseract 폐기, **GPT-4o-mini 비전 채택**(날짜 0/7 vs 6/7). 상세 위 Open Questions, `ocr-accuracy-test.png`
- [x] 🔴 **`/api/label/parse`** ✅ — `ocr.js`가 GPT 비전으로 제품명·재료·용량·유통기한 읽음. **연도 추론**(월.일만 있으면 채움, 실측: "05-08"→2027-05-08). 제조일/유통기한 둘이면 나중 날짜. 날짜 못 읽으면 재료 프리셋 폴백. 검증 `node scripts/test-parse.js`
- [x] 🔴 **촬영 → OCR → 확인 대기 큐** ✅ — `PhotoCaptureView`가 file input(capture)로 여러 장 촬영/업로드(클라서 2048px로 줄여 전송, 정확도 유지). 각 사진 → `status='pending'` 재고 생성. **확인 전엔 냉장고·레시피에 안 섞임(격리)**. 촬영 안내 문구는 실측서 도출
- [x] 🟡 **확인 화면 + 수정** ✅ — 카드로 읽은 값 검증, "고칠게요"로 제품명·유통기한(날짜피커)·보관 수정 후 확정. E2E: 실사진 2장 촬영→판독→수정→냉장고 편입 확인
- [ ] 🟡 **촬영 30초 기준선 실측** — 실제 장바구니로 (베타에서). ⚠️ 현재 OCR ~2~4초/장(동기). 진짜 백그라운드 처리는 베타 개선 항목
- [ ] 🟡 (베타 개선) 사진 원본 저장(확인 카드에 사진 표시), price/무게 혼동 프롬프트 개선, 백그라운드 판독
...
- [x] 🟢 **멀티 마켓 구매 링크** ✅ — `/api/buy-links`가 쿠팡·마켓컬리·이마트몰·네이버쇼핑 4개 마켓 검색 딥링크 생성. 재료마다 마켓 골라 직접 비교(중립성 해자 — 쿠팡이 못 하는 자리). ⚠️ **실시간 가격 비교는 불가**(쿠팡 가격API 없음+네이버가 쿠팡 배제) → 딥링크 + 네이버 참고가(키 있을 때). URL은 `MARKETS` 한 곳에서만 생성 → 제휴 승인 시 딥링크로 교체. 상세: `MARKETPLACES.md`
...
- [x] 🟢 **베타 개선 1차** ✅ (실사용 피드백) — ①재료 담기 단위 선택(g/ml/개) ②음식명으로 레시피 생성(`/api/recipes/byname`) ③인분 선택+재료 투입량(레시피 `servings`, 상세서 비례 스케일) ④주문분(`status='ordered'`, '곧 도착' 섹션, 요리 '주문분 포함' 토글, 받으면 냉장고로). 검증 `node scripts/test-features.js` 17건(로컬·프로덕션 통과)
  - ⚠️ **정리 스크립트 주의:** `cleanup-test-users.js`는 @test.com/@t.com/@nowaste.app만 지운다. 실제 사용자 이메일을 EXACT 목록에 넣으면 진짜 데이터가 지워진다(한 번 사고)
...
- [ ] 🟡 **쿠팡파트너스 + 네이버 쇼핑커넥트 가입 신청** — 배포된 앱 URL로 (둘 다 앱 필요, 병행 가능)
- [ ] 🟡 승인 후 `MARKETS`의 쿠팡·네이버 URL을 **제휴 딥링크**로 교체 (키는 `.env`에만). **프론트 수정 없음**
- [ ] 🟢 **마켓컬리·SSG 제휴 문의** → 되면 `affiliate=true`. 트래픽 후 링크프라이스로 11번가·G마켓 추가 (`MARKETPLACES.md`)
- [ ] 🟢 (선택) `NAVER_CLIENT_ID/SECRET` 발급 → 구매 시트에 **네이버 참고 최저가** 자동 표시
- [ ] 🔴 **Capacitor로 안드로이드 앱 감싸기** — `npx cap add android`. 웹 코드 그대로 사용
- [ ] 🟡 **온디바이스 OCR로 교체** — `@capacitor-community/image-to-text` (iOS Vision / Android ML Kit). **무료·오프라인·한글 정확.** GPT 비전 비용(장당 ~4원)이 여기서 ₩0이 된다
...
| `OPEN_AI_API` | 레시피 생성(`gpt-4o-mini`) + OCR(비전) | platform.openai.com → API keys. **`server.js`가 프록시하므로 프론트 노출 없음.** 레시피 캐시로 0 수렴, OCR 장당 ~4원(COST.md) |

### 선택 (있으면 기능 강화)

| 항목 | 설명 | 획득 방법 |
|------|------|----------|
| `NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET` | 구매 시트의 **네이버 참고 최저가** | developers.naver.com → 검색 API 신청(무료). 없으면 딥링크만 표시(우아하게 degrade) |
